...
- **Package shape:** Installable complementary module at `packages/bmad-multi-user/` (PACKAGE_ROOT) = skills + scripts + template `_bmad/custom` hooks; run scripts via `uv run`. Single gate evaluator CLI: `packages/bmad-multi-user/scripts/gate_eval.py`.
